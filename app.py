...
SYSTEM_PROMPT = (
    ...
    "ALWAYS respond in the SAME language the user writes in (Russian if they write Russian). "
    "NEVER translate the name 'Pulse' — always keep it as 'Pulse'.\n\n"
    "Use the step format ONLY when solving math, physics, chemistry or similar exact science problems:\n\n"
    "[STEP 1: step name]\n"
    "explanation\n\n"
    "[STEP 2: step name]\n"
    "explanation\n\n"
    "[ANSWER]\n"
    "final answer\n\n"
    "For all other questions (general knowledge, advice, history, literature, programming, etc) — "
    "answer naturally without any steps or special formatting.\n\n"
    "Math rules when using steps:\n"
    "- When moving a term to the other side of equation, CHANGE its sign\n"
    "- Always show every calculation step\n\n"
    "Rules:\n"
    "- Never say you are Llama, GPT or any other AI model\n"
    "- If asked who you are: say exactly 'Я Pulse' (keep name as Pulse)\n"
    "- If asked who made you: say 'Команда Pulse' (keep name as Pulse)"
)
...
def telegram_auth():
    try:
        ...
        new_chat = Chat(user_id=user.id, title='Новый чат')
...
def google_callback():
    try:
        ...
        if not user:
            ...
            db.session.add(user)
            db.session.commit()
        session['user'] = {'id': user.id, 'email': user.email, 'name': user.name, 'picture': user.picture}
        session.permanent = True
        new_chat = Chat(user_id=user.id, title='Новый чат')
        db.session.add(new_chat)
        db.session.commit()
...
def new_chat():
    if not session.get('user'):
        return jsonify({'error': 'Not logged in'}), 401
    nc = Chat(user_id=session['user']['id'], title='Новый чат')
    db.session.add(nc)
    db.session.commit()
...
def index():
    ...
    if session.get('user') and session.get('current_chat_id'):
        ...
        if chat and chat.messages:
            ...
            context_storage[session_id] = [{"role": "system", "content": SYSTEM_PROMPT}] + msgs
        user_dict = session.get('user')
        if user_dict and user_dict.get('email') is None:
